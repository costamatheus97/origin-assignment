...
const TotalAmount: React.FC = () => {
  const { setTotalAmount, totalAmount } = useCalculator();

  return (
    <Container className="totalAmount">
      <label htmlFor="inputContainer" className="totalAmount__label">
        Total amount
      </label>
      <InputContainer className="inputContainer" id="inputContainer">
        <div className="inputContainer__wrapper">
          <svg
            className="inputContainer__icon"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
        ...
        <input
          ...
          type="text"
          onChange={event =>
            setTotalAmount(event.target.value.replace(/[, ]+/g, ''))
          }
        />
      </InputContainer>
    </Container>
